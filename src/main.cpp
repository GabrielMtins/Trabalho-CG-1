 ...
 * Ele utiliza somente as bibliotecas padrão (STL), glfw (gerenciamento de janelas),
 * GLAD (carregamento de funções de OpenGL) e funções matemática (glm).
 *
 * A glm não está incluída no projeto, pois ela é uma biblioteca grande (25MB) e header only, 
 * mas basta clonar o repositório do github na
 * pasta external/, de forma que fique external/glm/(repositório). Ou seguir os passos 
 * no README.md e adicionar via submodules.
 *
 ...
 *
 *  A cena representa um desenvolvedor nos anos 90 testando as funções de escala
 *  do super nintendo. Na mesa, há um livro de programação em C (escrito C nele, construído com
 *  primitivas), há um Super Nintendo e um monitor.
 *  No plano de frente, há um controle de Super Nintendo, que o desenvolvedor
 *  utiliza para testar a função de escala do videogame.
 ...
 *
 *  Os objetos são complexos (feitos em código com auxílio da classe Builder).
 *  Cada objeto tem sua matriz de transformação (model) própria.
 *  Cada objeto tem utiliza variedades de translações, rotações e escalas.
 *
 *  O teclado é utilizado para translação do livro.
 *  O teclado é utilizado para translação do botão do controle e para a escala do pacman.
 *  O teclado é utilizado para rotação do controle.
 *  Os objetos fazem sentido na cena, como dito anteriormente.
